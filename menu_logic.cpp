...
static uint8_t selectedWiFiItem = 0;  // 0 = SETUP, 1 = BACK
...
void menu_handle_event(EncoderEvent_t event) {
  ...
  switch (currentState) {
    ...
    case MENU_STATE_WIFI:
      if (event == EVENT_ENCODER_LEFT || event == EVENT_ENCODER_RIGHT) {
        // Поворот: переключаем между SETUP (0) и BACK (1)
        if (event == EVENT_ENCODER_RIGHT) {
          selectedWiFiItem = (selectedWiFiItem + 1) % 2;
        } else {
          selectedWiFiItem = (selectedWiFiItem == 0) ? 1 : 0;
        }
        updateWiFiSelection(selectedWiFiItem);
      } else if (event == EVENT_BUTTON_CLICK) {
        if (selectedWiFiItem == 1) {  // BACK
          currentState = MENU_STATE_TOP;
          selectedItem = 1;  // подсветка на WIFI в главном меню
          drawMenuTop(selectedItem);
        }
        // Для SETUP клик пока ничего не делает (ждём HOLD_RIGHT)
      } else if (event == EVENT_HOLD_RIGHT) {
        if (selectedWiFiItem == 0) {  // SETUP
          startWiFiConfig();          // Эта функция НЕ ВЕРНЁТ УПРАВЛЕНИЕ
                                      // (будет перезагрузка)
        }
      }
      break;
